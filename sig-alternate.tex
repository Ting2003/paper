...
\begin{comment}
	...
	As the size of power grids grows, simulation time will become more and more, and even be unacceptable. This is true even
	for a efficient serial solver. Besides, the memory bottleneck is also a problem. There is not enough memory to hold the large 
	size data. To address the simulation time issue, parallel methods should be taken into consideration. For example, Domain 
	Decomposition method can be used to simulate power grids in parallel. Furthermore, parallel architecture can
	be utilized to overcome the memory bottleneck. For example, in distributed memory system, more processors will bring bigger memory
	size. As a result, implementing a parallel method is a promising way to achieve an efficient solver for 
	large-scale power grids analysis.\\ 
\end{comment}
